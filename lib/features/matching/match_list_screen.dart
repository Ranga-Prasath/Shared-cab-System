...
import 'package:shared_cab/data/dynamic_match_generator.dart';
...
class _MatchListScreenState extends ConsumerState<MatchListScreen> {
  ...
  Future<void> _searchMatches() async {
    // Generate dynamic matches based on the user's actual route
    final rideRequest = ref.read(currentRideRequestProvider);
    if (rideRequest != null) {
      try {
        final results = await DynamicMatchGenerator.generateMatches(
          rideRequest.pickup,
          rideRequest.dropoff,
        );
        if (!mounted) return;
